...
short knight(const char *p1, const char *p2) {
    ...
    enqueue(&q, (struct position) {.x = x1, .y = y1});  // add starting position to queue
    ...
    while (!is_empty(&q)) {  // continue until queue is empty
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (p.x == x2 && p.y == y2) {  // check if current position is the destination
                return steps;  // return number of steps if destination is reached
            }
            for (int j = 0; j < 8; j++) {  // try all possible knight's moves
                int x = p.x + dx[j];
                int y = p.y + dy[j];
                if (x >= 0 && x < 8 && y >= 0 && y < 8 && !visited[y][x]) {  // check if move is valid
                    enqueue(&q, (struct position) {.x = x, .y = y});  // add new position to queue
...
int main(void) {
    ...
    assert(steps == 1);
    printf("From a3 to b5 a minimum of %d steps have been calculated (expected 1).\n", steps);
    // test 2: start and end positions are on opposite corners of the board
    steps = knight("a1", "h8");
    assert(steps == 6);
    printf("From a1 to h8 a minimum of %d steps have been calculated (expected 6).\n", steps);
    // test 3: start and end positions are on the same rank
    steps = knight("b7", "e4");
    assert(steps == 2);
    printf("From b7 to e4 a minimum of %d steps have been calculated (expected 2).\n", steps);
    // test 4: start and end positions are on the same file
    steps = knight("c3", "c6");
    assert(steps == 2);
    printf("From c3 to c6 a minimum of %d steps have been calculated (expected 2).\n", steps);
    // test 5: start and end positions are the same
    steps = knight("a1", "a1");
    assert(steps == 0);
    printf("From a1 to a1 a minimum of %d steps have been calculated (expected 0).\n", steps);
    // test 6: start and end positions are separated by an odd number of ranks and files
    steps = knight("d4", "e6");
    assert(steps == 2);
    printf("From d4 to e6 a minimum of %d steps have been calculated (expected 2).\n", steps);
    // test 7: start and end positions are separated by an even number of ranks and files
    steps = knight("a2", "c4");
    assert(steps == 2);
    printf("From a2 to c4 a minimum of %d steps have been calculated (expected 2).\n", steps);
    // test 8: start and end positions are not connected
    steps = knight("a1", "h7");
    assert(steps == -1);
    printf("From a1 to h7 a minimum of %d steps have been calculated (expected -1).\n", steps);
    return 0;
}
